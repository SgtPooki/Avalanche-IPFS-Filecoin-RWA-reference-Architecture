...
export function AssetScreen({ assetId, label, owner, clients, onVerify }: VerifyScreenProps & { onVerify: () => void }) {
  ...
  return <section>
    <div className="screen-head"><div className="eyebrow">Property</div><h1>{label}</h1><p className="sub mono">{assetId}</p></div>
    ...
    <div className="record-identity">
      <dl className="kv"><dt>Recorder</dt><dd className="mono">{owner}</dd><dt>Registry</dt><dd className="mono"><a href={`${registry.explorer}/address/${registry.address}`} target="_blank" rel="noreferrer">{registry.address}</a></dd></dl>
      <p className="sub">Only the recorder's account can add versions under this parcel record.</p>
    </div>
    {result.state === 'loading' && <p role="status">Reading the property's current records...</p>}
    {result.state === 'failed' && <p role="alert">Could not read the property record: {result.message}</p>}
    {result.state === 'ready' && (result.value.anchor == null || result.value.manifest == null ? <p>No versions published.</p> : <>
      <h2>Recorded documents, version {result.value.anchor.version}</h2>
